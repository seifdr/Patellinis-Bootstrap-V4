...
class PatelinnisMenu
{
	...
	function order_cats(){
		...
		if( $_POST['type'] == "category" ){
			...
			foreach ( $_POST['catOrder'] as $key => $value ) {
				update_term_meta( $value, 'categoryOrder', $key );
			};
	...
	public function get_cats(){
		$postCats = get_terms( 'category', array( 'hide_empty' => false, 'exclude' => array('1'), 'orderby' => 'meta_value', 'meta_key' => 'categoryOrder', 'order' => 'ASC') );
				...
				<div class="col1of2">	
					...
					<?php 
						foreach ($postCats as $cats ) {
							//$x = get_term_meta( $cats->term_id, 'categoryOrder');
    ...
    function all_menu_items_on_blog( $query ){
    	// make sure all posts/menuitems show on menu
  		if( $query->is_main_query() && ( is_home() || is_archive() ) ){
  			...
  			$query->set('meta_key', 'menuItemOrder');
